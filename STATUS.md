...
2. Physical Windows PC session — install the CI-built `.msi`, pair a real Scanmarker Air over BLE, confirm a stroke round-trips through `RunOCR_V7` and produces real text (set Settings → Scanmarker email first per #1), then test both injection paths (ValuePattern in Notepad/browser, clipboard fallback in Word), confirm tray UI renders, confirm WebSocket bridge/extension handshake.
3. Same class of fix for USB serial (`usb_serial.rs`) if/when a Scanmarker Air over USB needs to be supported — currently still assumes plain-text protocol.
4. Investigate phantom BLE "Connected" state on Mac cold launch — could be masking real bugs, fix before hardware testing.
5. Real Scanmarker pen test on Mac — BLE connect, scan, verify full pipeline with actual hardware (biggest unverified surface on Mac; Mac's protocol implementation is believed correct per cross-reference, but never confirmed against physical hardware).
6. Browser extension end-to-end test (both platforms) — inject into Google Docs/Gmail via the extension, not just native AX/UIA.
7. Apple Developer Program enrollment → notarize Mac DMG (24-48h latency, start early, zero eng time blocking).
8. EV Code Signing Certificate for Windows installer.
9. Once both hardware sessions land, add a "confirmed working targets" matrix (per-app: Word, Notepad, Outlook, Chrome, etc.) to this doc.

---
Last updated: 2026-07-04 (Mac core pipeline live-verified this session; Windows BLE/OCR protocol rewritten to match real hardware and confirmed building on real Windows CI — run 28711940122 green on both platforms; not yet hardware-tested; settings UI for the new email field still needed)
